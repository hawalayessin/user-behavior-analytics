...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs">
      ...
      <p className="text-slate-100 font-semibold">{payload[0].value.toLocaleString()} drop-offs</p>
    </div>
  )
}
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex-[1]">
      ...
      <div className="mt-3 bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs p-3 rounded-lg">
        ⚠ Day 3 (48–72h) is the critical conversion decision point.
        Users who fail to convert by 72h show ~94% churn probability.
        Focus retention efforts on the first 3 days.
      </div>
    </div>
  )
}
