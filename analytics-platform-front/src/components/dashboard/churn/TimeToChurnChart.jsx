...
const BUCKETS = ["0-3 days", "4-7 days", "8-30 days", "31-90 days", "90+ days", "Unknown"]

export default function TimeToChurnChart({ data = [] }) {
  const rows = useMemo(() => {
    // Collapse across services: show stacked distribution per bucket
    const byBucket = new Map()
    for (const b of BUCKETS) byBucket.set(b, { bucket: b, VOLUNTARY: 0, TECHNICAL: 0 })
    for (const r of data) {
      const key = r.bucket ?? "Unknown"
      const cur = byBucket.get(key) ?? { bucket: key, VOLUNTARY: 0, TECHNICAL: 0 }
      const t = r.churn_type
      if (t === "VOLUNTARY" || t === "TECHNICAL") cur[t] += Number(r.count ?? 0)
      ...
      <ChartContainer className="h-80 w-full min-w-0">
        <ResponsiveContainer width="100%" height="100%" minWidth="0" minHeight="0">
          <BarChart data={rows} margin={{ top: 10, right: 18, left: 0, bottom: 0 }}>
            ...
            <XAxis dataKey="bucket" stroke="rgba(148,163,184,0.8)" tick={{ fontSize: 12 }} />
            ...
            />
            <Legend />
            <Bar dataKey="VOLUNTARY" name="Voluntary" stackId="a" fill="#F97316" />
