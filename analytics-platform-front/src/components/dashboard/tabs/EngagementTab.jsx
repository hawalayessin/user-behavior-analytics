...
function KPICard({ title, value, subtitle, icon: Icon, iconColor, alert }) {
  return (
    <div className={`bg-slate-900 rounded-xl border p-5 flex flex-col gap-3
...
export default function EngagementTab({ data }) {
  ...
  const stickinessMetrics = [
    ...
    },
    {
      ...
      value: users.total > 0 ? Math.round(users.active / users.total * 100) : 0,
      target: 70,
      color: "#6366F1",
    },
    {
      label: "Pipeline d'essai",
      value: subscriptions.total > 0
        ? Math.round(subscriptions.trial / subscriptions.total * 100)
        : 0,
      target: null,
      color: "#F59E0B",
    },
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
        ...
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={sparkData}>
            ...
            <XAxis dataKey="day"  tick={{ fill: "#64748B", fontSize: 11 }} interval={2} />
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={serviceBar} layout="vertical">
                ...
                <YAxis dataKey="name" type="category"
                       tick={{ fill: "#94A3B8", fontSize: 11 }} width={90} />
                <Tooltip
