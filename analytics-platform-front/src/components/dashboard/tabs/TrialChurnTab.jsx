...
export default function TrialChurnTab({ data, filters }) {
  ...
  const churnPieData = churnBreakdown
    ? [
        { name: "Voluntary", value: churnBreakdown.voluntary_pct ?? 0, fill: "#EF4444" },
        { name: "Technical", value: churnBreakdown.technical_pct ?? 0, fill: "#F59E0B" },
      ]
    : [
        { name: "Voluntary", value: churn?.voluntary_pct ?? 0, fill: "#EF4444" },
        { name: "Technical", value: churn?.technical_pct ?? 0, fill: "#F59E0B" },
      ]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: "Trial Conversion → Paid", value: `${subscriptions.conversion_rate_pct}%`, color: "text-emerald-400" },
          { label: "Monthly Churn",          value: `${churn.churn_rate_month_pct}%`,         color: "text-red-400"     },
          { label: "Voluntary Churn",       value: `${churn.voluntary_pct}%`,                color: "text-violet-400"  },
        ].map((kpi) => (
      ...
      <div className="bg-amber-500/10 border border-amber-500/20 text-amber-300 text-sm p-4 rounded-xl">
        ⚠️ <strong>Critical zone (48–72h)</strong> — Users who do not convert before 72h show
        ~94% churn probability. Focus retention efforts on the first 3 days of the trial.
      </div>
    </div>
  )
}
