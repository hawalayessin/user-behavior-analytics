...
export function useTimeToChurn({ start_date, end_date, service_id, churn_type = "ALL" } = {}) {
  ...
  const fetchData = useCallback(async ({ force = false } = {}) => {
    ...
    try {
      ...
      const payload = await getWithCache(`/analytics/churn/time-to-churn${qs ? `?${qs}` : ""}`, { force })
      setData(payload)
    } catch (err) {
