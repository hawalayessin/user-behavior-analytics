...
export default function FreeTrialBehaviorPage() {
  ...
  const kpis = useMemo(() => {
    ...
    return {
      ...
      converted_trials: kpiData.converted_trials ?? 0,
      cancelled_trials: kpiData.cancelled_trials ?? 0,
      dropped_trials: kpiData.dropped_trials ?? kpiData.cancelled_trials ?? 0,
    };
  }, [kpiData]);
  ...
  const exportExcel = async () => {
    ...
    XLSX.writeFile(
      wb,
      `trial_users_export_${new Date().toISOString().split("T")[0]}.xlsx`,
    );
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400">
            Analyze user engagement during trial period and conversion metrics
          </p>
        </div>
            ...
            {kpiLoading ? (
              ...
              <>
                ...
                <KPICard
                  ...
                  iconBg={
                    kpis.conversion_rate < 20
                      ? "bg-red-500/10"
                      : "bg-green-500/10"
                  }
                  trend={kpis.conversion_rate < 20 ? -1 : 0}
                  trendLabel={
                    kpis.conversion_rate < 20 ? "below target" : "on track"
                  }
              ...
              {kpiLoading ? (
                ...
                <SubscriptionDonutChart
                  data={[
                    ...
                    {
                      name: "Active",
                      value: kpis?.converted_trials ?? 0,
                      fill: "#10B981",
                    },
                    {
                      name: "Cancelled",
                      value: kpis?.dropped_trials ?? 0,
                      fill: "#EF4444",
                    },
                  ]}
                />
                ...
                <tbody className="divide-y divide-slate-700">
                  {trialUsersLoading ? (
                    ...
                    sortedTrialUsers.map((user) => {
                      const cfg =
                        TRIAL_STATUS_MAP[user.status] ??
                        TRIAL_STATUS_MAP.dropped;
                      const duration = user.trial_duration_days ?? 0;
                      const converted =
                        user.status === "converted" ? "Yes" : "No";
                      const convertedBg =
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <span
                              className={`inline-block px-3 py-1 rounded text-xs font-medium border ${convertedBg}`}
                            >
                              {converted}
                            </span>
                          </td>
